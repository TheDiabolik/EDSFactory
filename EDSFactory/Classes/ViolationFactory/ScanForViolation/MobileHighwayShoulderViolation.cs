...
{
    class MobileHighwayShoulderViolation : IScanForViolation
    {
        private static MobileHighwayShoulderViolation m_do; 
          DataSourceWatcher m_watcher;

          public MobileHighwayShoulderViolation()
        {
            m_watcher = new DataSourceWatcher();
            m_watcher.AddWatcher(MobileHighwayShoulder.Singleton(MainForm.m_mf));
        }


        public static MobileHighwayShoulderViolation Singleton()
        {
            if (m_do == null)
                m_do = new MobileHighwayShoulderViolation();
        ...
        {
            Settings.MobileHighwayShoulderSettings m_ses = Settings.MobileHighwayShoulderSettings.Singleton();
            DatabaseOperation.MobileHighwayShoulder se = DatabaseOperation.MobileHighwayShoulder.Singleton();
            ...
            List<string> workPlan = m_wpams.MobileHighwayShoulderWorkingPlan;

            //resimler klasöründeki resimleri alıyoruz
                //List<string> imageNameList = FileOperation.Find(m_ses.m_imagePath);
            ...
            List<string> imageNameList = new List<string>(taskDatabaseSelect.Result);   
                ...
                List<string> wantedImageTypeAndRangeList = CheckImageNameOrList.FindTypeAndRangeList(confirmedImageNameList, Enums.TypeAndRange.Wanted, 0180, 0199, "L1-C1", "L1-C2");
                ...
                foreach (string itemPlate in plate)
                {
                   ...
                   Enums.Validate.Valid, "L1-C1", "L1-C2");
                    ...
                    Task<List<DateTime>> checkDatabaseViolation = DatabaseOperation.MobileHighwayShoulder.Singleton().AsycSelect(itemPlate);
                    ...
                    if (violationImagesNames.Count > 0)
                    {
                        ...
                        for (int i = 0; i < violationImagesNames.Count; i += 2)
                        {
                            ...
                            //MainForm.m_syncImages.Remove(violationImagesNames[i]);
                            //MainForm.m_syncImages.Remove(violationImagesNames[i + 1]);
                        }
                ...
                if (m_ses.m_deleteImages)//plaka haricinde uygun olmayan formatta resim varsa
                {
                    ...
                    if (imageNameList.Count > 0)
                    {
                        ...
                        List<string> notWantedTypeAndRange = CheckImageNameOrList.FindTypeAndRangeList(imageNameList, Enums.TypeAndRange.NotWanted, 0180, 0199, "L1-C1", "L1-C2");
